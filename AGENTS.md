...
- Use test harnesses, fixtures, demo/dev modes, or explicitly assigned side-by-side non-production ports instead. If production smoke verification is genuinely required, ask for consent at that point and leave it pending when consent is not given.
