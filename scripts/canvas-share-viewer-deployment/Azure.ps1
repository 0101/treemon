...
function Get-AzureContext {
    ...
    $targetAccount = Invoke-AzJson -Arguments @(
        'account', 'show',
    ...
    if (-not [string]::Equals(
        ...
        [StringComparison]::OrdinalIgnoreCase)) {
        throw "Azure CLI is not currently selecting the requested subscription. Run 'az account set --subscription <subscription>' and retry."
    }

    if (-not [string]::Equals(
        [string] $targetAccount.tenantId,
        $Tenant,
        [StringComparison]::OrdinalIgnoreCase)) {
        throw 'The requested tenant does not own the selected subscription.'
    }

    if ([string] $targetAccount.state -cne 'Enabled') {
        throw 'The selected subscription is not enabled.'
    }
    ...
    [pscustomobject]@{
        SubscriptionId = [string] $targetAccount.id
        TenantId = [string] $targetAccount.tenantId
