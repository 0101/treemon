...
When an agent creates, updates, moves, or deletes a canvas doc, the **authoring session changes
ownership explicitly**: the extension — which holds that session's own `sessionId` — POSTs
`{worktreePath, filename, sessionId, remove}` to Treemon's `/api/canvas/attribute` endpoint. This
is authoritative because the change comes from the process that performed the file operation.
...
destination, and Delete targets remove the old ownership. Changes are single-shot and best-effort;
the existing explicit claim remains the recovery path for a failed request.
...
Ownership is stored as `Map<worktreePath, Map<filename, string>>` (worktree → filename → sessionId),
persisted to `data/canvas-owners.json` on every change, and loaded on server startup.
...
| `src/Extension/extension.mjs`, `src/Extension/canvas-ownership.mjs` | Detect lifecycle-aware canvas writes, stamp the session ID, and forward single-shot changes to `/api/canvas/attribute`; also expose the `canvas_take_ownership` escape hatch |
