...
- **Pure `reconcileMergedPrs`** (no I/O, unit-testable):
  ...
  (worktreeHeads: Map<string, Set<string>>) -> (knownBranches: Set<string> option) ->
  ...
  **identity-gates** by tip: a record is evicted when its non-empty `HeadSha` matches **none** of the
  branch's present worktree tips (a reused-name incarnation); a match against **any** observed tip, a
  missing/empty tip set, or an empty (legacy) `HeadSha` is kept (Decision #11 / review F2 — the tips
  are a SET per branch because several worktrees can track the same upstream branch at different
  tips). Pruning to `knownBranches` runs only when it is `Some`
...
- **Wiring in `src/Server/RefreshScheduler.fs`** (`RefreshPr` handler): after
  ...
  build `worktreeHeads` (upstream branch name → **set** of that branch's `GitData.HeadCommit` tips,
  via `Seq.groupBy` — one per worktree tracking the branch) for identity gating (Decision #11 / review
  F2), run `reconcileMergedPrs`, `setForRepo` only if changed, then `UpdatePr(repoId, effectiveMap)`.
...
| 11 | Branch identity for merged records (review F4) | Records key on branch **name**, which git lets you delete and recreate with unrelated history — so a reused name would resurrect a prior branch's merged badge (a false "Merged" on new work, and `sync` could `git push` it via Decision #9's gate). Each `MergedPrRecord` now carries **`HeadSha`**: the worktree tip commit at the moment the merged PR was recorded, sourced from **`GitData.HeadCommit`** (the `getLastCommit` hash we already compute but previously discarded). **Identity source is the worktree tip** — pure, no GitHub/wire-type change; the live PR map is already scoped to existing worktrees (`filterRelevantPrs knownBranches`), so a tip is always available when a record is written. **Overlay is gated by tip match:** the reconstructed merged `HasPr` is surfaced only when one of the worktree's current tips == the record's `HeadSha`. A record whose branch has a **present, non-empty tip set** that contains **none** of those tips is a confirmed different incarnation and is **evicted** from the store (self-cleaning). (The tips are a **SET** per branch, not a single tip — several worktrees can track the same upstream branch at different tips, so identity is "match ANY observed tip"; collapsing them to one arbitrary tip evicted records whose still-valid tip lost the collapse — review F2.) **Legacy fallback:** a record with an empty `HeadSha` (pre-existing on-disk data, or a tolerant-load default) is treated as unverified and keeps the prior name-only overlay until it is re-observed live and re-stamped — so no existing badge vanishes on deploy. `reconcileMergedPrs` stays **pure** (Decision #5): `RefreshPr` builds `worktreeHeads: Map<branch, Set<tipSha>>` from `repo.GitData` (grouping every worktree tip that tracks each upstream branch, review F2) and passes it in; the upsert stamps `HeadSha` from that set. **Exact match (not ancestry)** means a branch advanced past its own merge drops the badge — conservative, arguably correct (there is now unmerged work), and never worse than the pre-persistence `NoPr`. **Never write an unverifiable identity (review F1):** the upsert records a newly observed merge **only when the branch has a present, non-empty worktree tip**; if the tip is unknown at observation time (a transient `getLastCommit` failure leaves the branch out of `worktreeHeads`, or with `""`), the merge is **not** persisted and any existing record is left untouched — because an `HeadSha = ""` written now is treated as legacy and permanently exempted from identity eviction, so a later reused branch name could resurrect it. Empty-`HeadSha` **legacy** records already on disk are still tolerated by the gate (legacy fallback above); this only stops *writing* new empty-`HeadSha` records. |
...
| `src/Server/RefreshScheduler.fs` | `RefreshPr` handler reconciles live PRs with the store, persists on change, posts the effective map via `UpdatePr`. Collects read-failed worktree paths for `pruneScope` (Decision #10) and builds `worktreeHeads` (branch→**set of** tip SHAs, review F2) for identity-gated reconcile (Decision #11). |
