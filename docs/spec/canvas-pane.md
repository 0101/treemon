...
- The in-place target is `document.body.innerHTML`; authored `<head>` elements and attributes on `<html>` or `<body>` sit outside it. The controller captures the authored head and `<html>` attributes when its final injected head script installs, before `DOMContentLoaded` integrations can add live-only elements, then captures `<body>` attributes once the body is parsed. Incoming changes to that source shell reload the document, while later live mutations do not become false authored changes. This makes head stylesheet, title, metadata, document-theme, and body-class changes take effect without sacrificing dirty form state for a body-only edit. Styles and attributes on descendants inside `<body>` remain on the fast morph path.
