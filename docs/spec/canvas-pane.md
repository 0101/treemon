...
- The two kinds get materially different instructions. An `AgentDoc` session is told to claim the doc with `canvas_take_ownership` (starting a session does not by itself reassign ownership) and to respond by editing the file. A `SystemView` session is told **not** to edit the file — Treemon regenerates it from live data and would discard the changes — and not to claim it, since it has no author and the claim tool refuses one; it acts on what the user asked about instead.
