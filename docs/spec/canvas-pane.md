...
- **Exact-equality duplicate coalescing** — both queues compare payloads exactly on their existing bounded fields (server: prompt kind, text, filename, and target session; extension: transport kind and session prompt text). There is no revision metadata and no content normalization, and a payload stops being pending the moment it is handed to a bridge (`SessionBridge.appendPending`) or to `session.send` (`createSendQueue`, key deleted before the call so a rejected send leaves nothing stale). Suppression therefore only ever removes a message that would repeat an undelivered one.
