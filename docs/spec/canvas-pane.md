...
- A switch-back does **not** re-morph an unchanged doc. A morph re-fetches the on-disk HTML and idiomorph resyncs form fields (`<input>`/`<textarea>`/`<select>`) to that markup, which would wipe in-progress user input — so morphing an unchanged doc is not a harmless no-op. The switch-back morph is gated on a real on-disk change: while a doc is hidden, an on-disk `contentHash` change is recorded (it is not morphed in place, only the *active visible* doc is — Level 3), and only such a "stale" doc is morphed to catch up when it is next revealed. A tab switch with no on-disk change morphs nothing.
...
- **Change-gated switch-back morph** — switching to a previously hidden iframe morphs it **only when its on-disk content changed while it was hidden**, tracked by `CanvasState.StaleHiddenDocs` (a `Set<scopedKey * filename>`). At each `DataLoaded`, an `AgentDoc` whose `contentHash` changed but which is *not* the active visible doc (the active one is morphed in place immediately) is added to the set; `selectCanvasDoc` morphs a doc on switch-back only when it is in the set, then removes it. The earlier design dispatched `MorphActiveDoc` *unconditionally* on every switch-back on the assumption that an unchanged-content morph is a harmless no-op — but idiomorph resyncs `<input>`/`<textarea>`/`<select>` values to the re-fetched on-disk markup, so a gratuitous morph **wipes in-progress user input**. Gating on a real change keeps hidden iframes up to date without destroying form state on an ordinary tab switch.
