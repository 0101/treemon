...
- A document with an authored classic JavaScript, module, import map, or other browser-processed script reloads instead. A refetch returns source HTML rather than the post-execution DOM; morphing that source removes script-generated descendants, while scripts parsed through `DOMParser`/`innerHTML` do not execute when inserted. Explicitly rerunning arbitrary author code is unsafe because listeners, timers, observers, and other side effects have no general cleanup or idempotency contract. The controller checks both the live and incoming documents, so adding, retaining, changing, or removing an authored active script selects reload and tears down old effects. JSON and other inert data script blocks do not select reload.
- Every Treemon-injected runtime script carries `data-treemon-runtime` and is excluded from authored-script detection. A script-backed reload therefore replaces the document and installs the bridge, canvas messaging, selected-text actions, error overlay, Idiomorph, and morph controller once rather than rerunning them into the old page. The server stamps the raw-file content hash into each served document; after navigation, the newly loaded controller reports that hash through the existing completion message. The parent therefore advances only to the bytes that produced the new document, while a failed reload sends no completion and remains retryable.
