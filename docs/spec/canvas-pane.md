...
- `LaunchCanvasSession` uses the existing action-launch flow and includes the full on-disk doc path (`{worktree}/.agents/canvas/{filename}`) plus canvas context in the prompt, so the agent is pointed at the real file the doc server serves. That path is built once by `CanvasPrompt.continueWorking` in `src/Shared/Types.fs` — the single source of truth shared by the client launch and server auto-spawn flows. For an `AgentDoc` the prompt also instructs the started session to claim the doc with `canvas_take_ownership`, because starting a session does not by itself reassign ownership and the doc's interactions would otherwise keep waiting for the author that is gone. The instruction is gated on `CanvasDocKind`: the server's auto-spawn path launches only for a `SystemView`, which has no author to claim.
