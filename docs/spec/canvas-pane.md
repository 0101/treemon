...
| `src/Extension/extension.mjs`, `shutdown-endpoint.mjs`, `injection-request.mjs`, `session-prompt.mjs`, `send-queue.mjs` | Exact-process bridge registration, guarded prompt/shutdown endpoints, SDK session-ID compatibility, typed prompt transport, serialized send queue, heartbeat, and reconnect backoff |
