...
| Share button | yes | no (server-generated, not shareable) |
...
- **Freshness gate (restart-phantom suppression).** Canvas events and the idle auto-display target are gated by each doc's real file `LastModified` within a short freshness window (`canvasEventFreshnessMs`, 5 min), applied in `gateCanvasEventsByFreshness`/`isCanvasDocFresh` (`CanvasAwareness.fs`). Because `detectCanvasEvents` fires on content-hash *presence* deltas between polls, a pre-existing doc that merely reappears after an incomplete snapshot — notably the empty-`CanvasDocs` window right after a server restart — would otherwise be misreported as "just published"; gating on the real mtime drops those phantoms and restamps each surviving event's timestamp with the file's true modified time. This freshness window is deliberately separate from `canvasEventExpiryMs` (now 24 h), which governs only how long an already-shown notification lingers.
...
**Level 3 — Canvas-wide live update (general case):**
- On `contentHash` change, an AgentDoc without authored browser-processed scripts morphs in place (via injected idiomorph), so scroll position and focused-element identity stay intact. Immediately before the morph, the controller snapshots user-edited `<input>`/`<textarea>` values and dirty `checked` state on checkbox/radio inputs (current value or checked state differs from the authored default) and restores them afterward, including the active field's selection. Untouched fields still receive the new authored value or checked state, removed or retyped controls are not recreated, and restoration happens after mutation capture so user edits are not highlighted as an agent edit. The client sends a `content-updated` postMessage on hash change and keeps the iframe `src` stable for this static-document path.
- The in-place target is `document.body.innerHTML`; authored `<head>` elements and attributes on `<html>` or `<body>` sit outside it. The server hashes those surfaces directly from the raw authored HTML—the pre-body document shell plus the quote-aware body opening tag—and stamps that hash beside the content hash in the served document. The controller compares the stamped source hash with the refetch, so browser/runtime nodes inserted into the live head can never become false authored changes. For the same reason, its current-document script check is confined to `<body>`; authored head-script additions/removals are already represented by the incoming source and shell hash. A real shell change reloads, making head stylesheet, title, metadata, document-theme, and body-class changes take effect; styles and attributes on descendants inside `<body>` remain on the state-preserving morph path. When authored HTML contains neither a closing head nor an explicit body opening tag, Treemon treats the complete fragment as its shell and reloads for every edit rather than risk leaving browser-hoisted styles or metadata stale.
- A document with an authored classic JavaScript, module, import map, or other browser-processed script reloads instead. A refetch returns source HTML rather than the post-execution DOM; morphing that source removes script-generated descendants, while scripts parsed through `DOMParser`/`innerHTML` do not execute when inserted. Explicitly rerunning arbitrary author code is unsafe because listeners, timers, observers, and other side effects have no general cleanup or idempotency contract. The server stamps whether the raw authored body contained a browser-processed script, so a script that removes its own live node still selects reload when later source removes it; incoming body scripts are also checked after parsing. JSON and other inert data script blocks do not select reload.
- Every Treemon-injected runtime script carries `data-treemon-runtime` and is excluded from authored-script detection. A script-backed reload therefore replaces the document and installs the bridge, canvas messaging, selected-text actions, error overlay, Idiomorph, and morph controller once rather than rerunning them into the old page. The server stamps the raw-file content hash into each served document; after navigation, the newly loaded controller reports that hash through the existing completion message. The parent therefore advances only to the bytes that produced the new document. A failed refetch leaves the old controller available for retry; a navigation that lands on a browser or HTTP error page requires the iframe to be remounted.
- Poll refreshes may repeat the same live-update request until its completion reaches the parent. Once a controller starts a reload it ignores further requests, and the newly loaded controller immediately completes any request for its stamped content hash even if the body is still loading. A refetch that returns the already-loaded hash is likewise completed without another morph or reload. Slow authored scripts therefore cannot turn repeated synchronization into a reload loop.
...
- Live-update requests carry the worktree, filename, and target content hash. Completions instead carry the content hash of the exact raw file bytes returned by the refetch, not an echo of the requested hash, so another write during the request cannot be accepted as matching a stale target. The parent sends only to the matching active iframe; completion is accepted only from that iframe with matching worktree/filename identity and advances only that document's loaded hash to the served hash. A failed or superseded refetch leaves the old loaded hash intact, so the next visible synchronization retries instead of silently declaring stale DOM current.

**Changed-content highlight.** Preserving scroll makes a static-document update easy to miss, so
the morph marks what it changed. Script-backed updates reload and receive no changed-content tint.
`src/Extension/canvas-morph.js` wraps `Idiomorph.morph` in a `MutationObserver`;
...
- **A morph that changes nothing re-applies the previous set.** Redundant update signals can fetch
  a body identical to the one already rendered; that no-op must not clear a highlight no edit has
  superseded. A morph that *does* change something but leaves nothing to tint — a pure deletion —
  clears instead, so the tint never marks content the latest edit did not touch.
...
| File | Purpose |
|---|---|
| `src/Shared/Types.fs` | Shared canvas domain types, API methods, and the JSON document-identity/prompt builder used by both startup flows |
...
| `src/Client/CanvasUpdate.fs` | Canvas `update`-arm bodies extracted from `App.fs` (Toggle/SetPosition/Select/Open/Archive(+Result)/CopyPath(+Result+Reset)/Share(+Result)/ClipboardWriteResult/DismissClipboardNotice/Navigate/MessageReceived/SendResult/Dismiss/LaunchCanvasSession/Morph*), the shared canvas helpers (`activeVisibleDoc`, `reconcileMountedDocs`, `syncVisibleDocCmd`, `applyFocus`), and the `messageListener` subscription glue. App.fs delegates one arm → one function. Compiled after `AppTypes.fs` and before `App.fs`. Body extraction only — `update` stays one function (no sub-`Msg`/`Cmd.map`). |
