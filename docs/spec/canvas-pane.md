...
- Each AgentDoc filename has a persistent routing target in `CanvasDocOwnership.fs`, assigned from an authoring write or an explicit claim. A SystemView stores nothing; its interactions resolve to a live session per interaction.
...
- The `▶ Start session` button is shown when the active doc is an `AgentDoc` whose own `OwnerSessionId` is not live — the same per-doc signal as the liveness dot, not a worktree-wide "no live bridge" condition. An `AgentDoc` routes only to its recorded owner, so a doc whose author has exited must stay recoverable even while unrelated sessions are live in that worktree. A `SystemView` never shows the button (starting a session for a server-generated view is meaningless).
- `LaunchCanvasSession` uses the existing action-launch flow. Its prompt is the started session's **first message**, written for an agent with no prior context: it names the doc's on-disk path (`{worktree}/.agents/canvas/{filename}`), says why the session exists, tells it to read the file, and warns it off the two wrong moves — trying to serve or open the file (Treemon already renders it) and answering in the terminal instead of in the doc. Both kinds also say the queued interaction that triggered the launch arrives as the next message. The text is built once by `CanvasPrompt.forLaunch` in `src/Shared/Types.fs` — the single source of truth shared by the client launch and server auto-spawn flows — and branches on `CanvasDocKind`, so a new kind is a compile error rather than a silently wrong prompt.
- The two kinds get materially different instructions. An `AgentDoc` session is told to claim the doc with `canvas_take_ownership` (starting a session does not by itself reassign ownership), to load the **`canvas` skill** — which carries what a prompt cannot: `canvasSend`, `canvasExpand`, and how to read the `canvas-selection` / `expand-section` payload that is often its next message — and to respond by editing the file. The skill installs alongside the canvas-bridge extension (`treemon.ps1`), so it is present wherever `canvas_take_ownership` is. A `SystemView` session is told **not** to edit the file — Treemon regenerates it from live data and would discard the changes — and not to claim it, since it has no author and the claim tool refuses one; it acts on what the user asked about instead. It is deliberately **not** sent to the `canvas` skill, which is an authoring contract: pointing a session that must not author at it would invite the edit the same prompt forbids.
- Canvas messages route to the author session for the selected doc.
- If the author session is unreachable, the message queues. It is delivered to whichever session owns
  the doc when a drain next runs — the author if it returns, or the session that claimed the doc via
  `▶ Start session`. Doc identity never changes.
