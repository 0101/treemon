...
- The in-place target is `document.body.innerHTML`; authored `<head>` elements and attributes on `<html>` or `<body>` sit outside it. The controller compares those surfaces with a source signature captured after load, excluding Treemon-owned marked nodes, and reloads when they differ. This makes head stylesheet, title, metadata, document-theme, and body-class changes take effect without treating user-driven live root mutations as authored changes. Styles and attributes on descendants inside `<body>` remain on the fast morph path.
