...
| 13 | **Split `WorktreeDiff.fs`** — it is over roughly 1,000 lines and mixes entry parsing and untracked-content handling with comparison orchestration. Extract those responsibilities so the file owns comparison orchestration and result types only. | `docs/spec/worktree-diff-viewer.md` | Idea |
