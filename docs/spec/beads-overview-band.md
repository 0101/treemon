...
| **Underway** | All work an agent has started: tasks with status `in_progress` (`Beads.InProgress`) **plus** open tasks under an **in_progress** feature (`Planning.Queued`, execution underway, next-up) — counted only **on a worktree with an active agent** (`CodingTool` = `Working` or `WaitingForUser`). On an inactive worktree it folds into **Unattended**. |
| **Blocked** | Tasks with status `blocked` (`Beads.Blocked`) — an **explicitly declared** status, never inferred. A task merely waiting on an unmet dependency is still `open` and counts as **Planned** (or **Underway**), because `blocks` edges are deliberately ignored (see decision (e)). |
