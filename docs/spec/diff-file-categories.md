...
- Names are trimmed before they are stored, displayed, or placed in `categoryPath`. Sibling names must be non-blank and unique after trimming with ordinal case-insensitive comparison. Root nodes are depth 1; the maximum path depth is 4. A repository may contain at most 200 nodes, a name at most 100 UTF-16 code units after trimming, a leaf at most 50 patterns, and a pattern at most 200 UTF-16 code units; exceeding any bound is invalid.
...
- Each category renders a header button carrying `aria-expanded`, its name, and the count of files in its whole subtree. Branch headers nest their children; leaf headers contain the existing file rows. Category names are repository-authored and are rendered as text, never as markup. The header's accessible name is its names followed by that count, so the reader of a screen reader hears the group rather than the punctuation between its parts.
- A category that holds exactly one child and no files of its own renders together with that child as a single row labelled with their names joined by ` > `, and a longer run collapses the same way, at every depth. The joined row *is* the deepest category of the run: it holds that category's child headers or file rows, is indented and reached by keyboard as the one row it renders as, and every disclosure rule reads that flattened shape. A category with two or more children, or with both a child and files of its own, keeps its own row.
...
- Explicit expand/collapse choices survive Refresh and layer-filter changes within the same page instance and are keyed by a collision-free serialization of the category path, not by joining names with a delimiter. A joined row is keyed by its deepest category's path, so a choice made on it stays with those files when a new sibling later breaks the run into separate rows. They are not persisted to browser storage, so reopening the view returns to the computed default.
...
- Activating it sends the fixed message `configure-diff-categories` through the SystemView canvas transport. Treemon's existing routing delivers it to the worktree's most recently active reachable session, or queues it and starts one when nothing is reachable, and the existing waiting and delivery-error banners report progress. An unreachable session is never resumed to receive it, per the routing spec's no-resume rule.
- The request text is fixed in the viewer script. It instructs the agent to analyze the repository, locate the root worktree that owns the shared `.treemon.json`, preserve every existing field, and modify only `diffCategories` using ordered repository-relative globs covering production, test, documentation, and instruction areas without catch-all patterns, because Treemon supplies **Other**. It states the non-empty node schema, the four-level bound, sibling-name uniqueness, the reserved top-level `Other` name, and the supported `?`, `*`, and `**` glob subset. It also says that array order decides display as well as precedence — groups are shown top to bottom in configuration order at every level — and asks the agent to order every level so a reviewer reads what matters most about a change before its incidental parts. It closes by telling the agent to verify its own work with `tm categories` rather than reasoning the glob subset out by hand, to fix a group that matches nothing and to treat a large unmatched count as patterns that miss part of the repository rather than as a reason for a catch-all, and to report the per-group counts and the unmatched count. No part of the request is derived from repository content, and no path is supplied by the browser: it names no expected category, ordering, or count, so the generated `diff.html` stays byte-identical across repositories. Because that prose duplicates rules the validator enforces, a unit test pins the viewer script's depth bound and reserved `Other` name to `DiffCategories.maxDepth` and `DiffCategories.reservedTopLevelName` (both `internal` for that reason), so the two cannot drift apart silently.
...
- While the request is outstanding the action shows a spinner in place of its icon and is disabled, so it reports that work is under way and cannot be triggered again. Both copies of the control — toolbar and warning — share this state, including a copy rendered while the wait is already running. A message the transport reports as undelivered never starts the wait.
- The viewer resolves the wait by watching the repository's categorization, not the agent: an agent can end its turn without writing anything, and what the view must react to is the configuration changing. It polls the categorization until its revision differs from the one the loaded summary carried, then reloads the summary so the new grouping appears without the user pressing Refresh. Polling stops on the first observed change or after a bounded wait, after which the control returns to its normal enabled state; a failed poll is retried rather than treated as a failed configuration. The wait belongs to the page instance, so a reload starts over.

### Checking a configuration from the command line

- `tm categories [path]` reports what a repository's categorization actually does, so the agent that just wrote `diffCategories` can check its own work instead of reasoning the glob subset out by hand. `path` is optional and defaults to the current directory; a path outside every watched root is an error naming the path.
- The report states the configuration state. **Missing** and **Invalid** exit non-zero — Invalid quoting the same fixed validation reason the viewer's warning carries — so the command is usable as a check and not only as a printer. **Configured** exits zero.
- A configured repository lists **every declared leaf** as its full root-to-leaf category path with the number of tracked files that classify into it, in configuration order, and then how many tracked files no leaf matched. A leaf whose count is zero is flagged in place: a configuration that is valid but matches nothing is otherwise a silent failure, because neither the server nor the viewer treats an empty group as wrong.
- Counts are measured against the tracked files of the owning repository root — the same root whose shared `.treemon.json` was read — rather than against the changed files of a diff, so the answer never depends on there being an interesting diff and is the same from every linked worktree.
- Category names are repository-authored, so the CLI strips control characters from them before they reach the terminal, as it already does for branch and PR text.

## Technical Approach

`DiffCategories` owns the category concept end to end: the parsed model, validation, glob compilation, and classification. It reads `diffCategories` through `TreemonConfig`, which remains the only reader of `.treemon.json`. The accessor distinguishes a missing file or key from malformed JSON and returns an owned value (or parses through a callback while the document is alive), never a `JsonElement` borrowed from a disposed document. The parsed model is a discriminated union so a branch and a leaf cannot be confused, and the configuration result distinguishes `Missing`, `Invalid`, and `Configured` rather than collapsing a malformed file into "no categories". Each leaf pattern compiles once per read into a matcher for the bounded glob subset. Matching is a direct bounded algorithm: the pattern is simulated as a set of reachable positions, first over path segments and then over the characters of one segment, so a star-crowded pattern costs pattern length times input length instead of backtracking, and repository text is never executed as regular-expression syntax. Compilation collapses a run of consecutive stars into one — in both alphabets, since a run means exactly what a single star means — which is what holds that linear bound for a pattern such as `a***b` or `src/**/**`; it also records a wildcard-free segment as a literal and, for a pattern free of `**`, the exact path depth it can match, so the dominant literal and fixed-depth cases decide with an ordinal comparison instead of the automaton. `DiffCategories` exposes classification and grouping as one pass over the changed files, so patterns compile once per summary rather than once per file and the caller cannot order files by a rule the classifier does not know.

The canvas server already resolves a request's worktree from one scheduler snapshot. That lookup also yields the owning `RepoId`, whose value is the normalized repository root, so the server reads the categorization for that root and passes it to the diff summary handler paired with the comparison context in a single `DiffSummaryTarget`, so a context and a categorization from two different repositories are not representable. The read stays in the `diff-summary` branch rather than in the shared worktree resolution, so requests for the viewer document, beads data, or a single file never touch `.treemon.json`. The existing diff path is unchanged: its Git enumeration, layer composition, 1,000-path cap, deadlines, and identity semantics keep their current behavior.

The `tm categories` check is server-mediated like every other CLI command, so `src/Cli` keeps referencing only `Shared`: it calls `getDiffCategoryReport` on `IWorktreeApi` and renders the returned `DiffCategoryReport`, which carries the state and, when configured, one entry per declared leaf plus the unmatched count. The server resolves the owning repository from one scheduler snapshot through `RefreshScheduler.tryFindOwningRepo` — the same lookup that gives a diff request its owning `RepoId`, and the reason a linked worktree resolves to the root it shares `.treemon.json` with — then reads the categorization and enumerates tracked files with `git ls-files` through the existing diff Git plumbing rather than a second process path. Counting runs through `classifyAndOrder`, so one matcher stands behind both the viewer's grouping and the report; because classification can only report the leaves something matched, `DiffCategories.leafPaths` supplies the declared outline that turns a leaf matching nothing into a visible zero.

Classification happens in the summary handler, after the diff service returns entries and before opaque identities are issued, so the stored viewer snapshot and the browser agree on both order and grouping. Every browser-facing file gains a `categoryPath` array, empty for unmatched files and for every file in Missing or Invalid mode. Every ready summary gains `"categorization": { "status": "missing" | "configured" | "invalid", "reason": string | null, "revision": string }`; `reason` is non-null only for Invalid. The revision is a short digest of the *validated outline* — names, nesting, order, and patterns — so reformatting `.treemon.json` is correctly not a change while a rewrite that keeps the `configured` status is. The same value is served on its own by `diff-categorization`, which performs the identical per-request read and runs no Git command, so a viewer waiting for an agent to write `diffCategories` can watch for the change without paying for a diff. One serializer produces both, so the polled shape and the summary's cannot drift. Raw patterns are never sent to the browser: the server owns validation and matching, and the browser receives only ordered structure plus display labels. Because the server emits files already ordered by group, the viewer reconstructs the tree of *present* categories from consecutive category paths and never renders a configured category that has no changed files.

The generated `diff.html` stays byte-identical across repositories, so `DiffProvisioner` keeps its exact-content synchronization: all repository-specific data arrives at runtime in the summary response. The document carries only structure; the viewer's stylesheet and script are self-hosted assets embedded in the server assembly and served by the canvas host alongside the pinned diff2html bundle, so category styling and rendering live in `viewer.css` and `viewer.js` rather than inline. The viewer builds its category tree from the response, flattens every run of single-child, file-less categories into one node before anything else reads that tree, renders nested sections, computes initial disclosure, and merges that with a page-instance map of explicit toggles keyed by category path. Each category is one section holding its header button plus a sibling panel of child sections or file rows, so collapsing a header hides exactly that subtree and a depth custom property supplies indentation without per-level rules. Because that structure is regular, a row's reachability is the `aria-expanded` chain of its ancestor headers — the same source of truth the collapsing CSS rule uses — so keyboard navigation and selection restoration both read disclosure rather than measured layout. The `Invalid` warning is one fixed viewer sentence completed by the server-supplied reason, which keeps repository-authored text out of the message while giving the configure action a place to attach. The single-open file accordion is unchanged and simply becomes nested inside category sections; category disclosure is a second, independent dimension layered around it, and collapsing a header that contains the open file runs the existing collapse path so the in-flight patch request is aborted and a late response cannot render into a hidden panel.
...
- **Single-child runs are flattened rather than rendered level by level**: a category with one child and no files of its own offers the reader no choice — opening it can only reveal one more row — so each such level costs an indent and a click for no information. Joining the names keeps the declared structure legible while the outline spends rows only where it actually branches. The joined row keeps the deepest category's identity because that is the node whose contents it discloses: its files, its count, and the toggle key that must survive a re-render.
...
- **Reuse of the SystemView canvas message**: `sendCanvasMessage` already resolves the target, queues, launches, and reports failure, so a categorization request needs no new action kind, prompt builder, dashboard control, or process-launch path.
...
- **Coverage over tracked files, not over a diff**: what a configuration author needs to know is whether the patterns cover the repository, and that question must be answerable the moment the configuration is written. Classifying a diff would make the answer depend on which files happen to have changed and would report nothing at all for a clean worktree.
- **The declared outline is reported, not only what matched**: classification can only name a leaf some file reached, so a report built from it alone would show a mis-typed pattern as a missing line rather than as a zero. Listing every declared leaf makes the failure the command exists to catch impossible to overlook.
- **The exit code reports the configuration state, not the counts**: Missing and Invalid are objective failures the server already distinguishes, so they are the non-zero cases. Whether a zero-match leaf is a mistake is the author's judgement, so it is flagged in the report rather than turned into a failed command.
- **The configure prompt sends the agent to `tm categories` rather than having it check its own globs**: the supported subset has corners an agent reasons past — above all `**` matching zero path segments — so a prose self-review regularly concludes a pattern covers files it does not. The command runs the same matcher the viewer groups with, so its counts are the answer, and asking for them back is what turns "valid but matches nothing" into something the reader of the report sees.
...
- `dotnet test src/Tests/Tests.fsproj --filter "FullyQualifiedName~DiffCategoryTests"` covers nested rendering, single-child chain flattening, initial disclosure, explicit toggle persistence within a page instance, selection restoration through collapsed ancestors, delayed patch completion after ancestor collapse, keyboard navigation over visible rows, and the configure action. The fixture lives in `src/Tests/DiffCategoryTests.fs` over the shared `src/Tests/DiffViewerTestHarness.fs`, which `DiffViewerTests` inherits too, so both viewer fixtures route the template and pinned assets through one setup path. Tests that script what successive Load/Refresh/reload requests see call the harness's `RouteSummaries`, which owns the only sequence cursor.
- `dotnet test src/Tests/Tests.fsproj --filter "FullyQualifiedName~DiffCategoriesCoverage|FullyQualifiedName~DiffCategoryReport|FullyQualifiedName~FormatDiffCategoryReport"` covers the `tm categories` check: counting over a real Git fixture's tracked files, zero-count leaves surviving into the report, Missing and Invalid states, and the rendered output's ordering, totals, exit codes, and control-character stripping.
...
| `src/Server/WorktreeApi.fs` | `getDiffCategoryReport`: resolves the owning repository and builds the coverage report |
| `src/Server/WorktreeDiff.fs` | Enumerates the owning repository's tracked files through the bounded Git process path |
| `src/Cli/Program.fs` | `tm categories`: renders the coverage report and sets the exit code |
| `src/Shared/Types.fs` | `DiffFileSummary.CategoryPath`, `DiffCategoryReport` |
| `src/Server/DiffTemplate.html` | Viewer document structure hosting the category hierarchy |
| `src/Server/Assets/diff/viewer.js` | Nested category rendering, disclosure, collapse state, configure action |
| `src/Server/Assets/diff/viewer.css` | Category header, nesting, and warning styling |
