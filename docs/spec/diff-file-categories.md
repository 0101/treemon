...
- `dotnet test src/Tests/Tests.fsproj --filter "FullyQualifiedName~DiffCategoryTests"` covers nested rendering, initial disclosure, explicit toggle persistence within a page instance, selection restoration through collapsed ancestors, delayed patch completion after ancestor collapse, keyboard navigation over visible rows, and the configure action. The fixture lives in `src/Tests/DiffCategoryTests.fs` over the shared `src/Tests/DiffViewerTestHarness.fs`, which `DiffViewerTests` inherits too, so both viewer fixtures route the template and pinned assets through one setup path.
