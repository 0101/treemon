...
- Every non-archived worktree card has an icon-only **Diff** action with an accessible label and tooltip. It remains disabled with a "Diff view not ready" tooltip until the generated `diff.html` SystemView appears in the scanned canvas-doc inventory; activation revalidates that inventory before opening and targeting the view. The same view can open in a standalone browser tab.
- The view loads a changed-file summary first, then restores the previous file selection or selects the first file. Changed files form a keyboard-operable, single-open accordion: for every non-empty ready summary, exactly one path header is expanded and exactly one patch is mounted. The accordion content is constrained to the available workspace height and owns vertical scrolling, so every file header and expanded patch remains reachable. A clean worktree shows an explicit empty state.
- Three inclusion checkboxes appear above the accordion: **Already committed**, **Local changes**, and **Untracked**. First use selects Already committed and Local changes but not Untracked. Later selections persist per worktree in browser storage. Changing a checkbox refreshes only that page instance's summary and identity snapshot.
...
- The file navigator is a single-open accordion because file paths and content form one browsing flow; exactly one loaded patch remains visible at a time.
...
- Run `dotnet test src/Tests/Tests.fsproj --filter "FullyQualifiedName~DiffViewerE2ETests" --logger "console;verbosity=normal"` to verify accordion behavior, wrapping and gutter geometry, syntax-highlighting success/failure/retry, status indicators, and toolbar controls in Chromium. Any horizontal overflow, overlapping gutter/code rectangles, multiple mounted patches, ready state without token markup, or inaccessible control/status fails the suite.
...
- After `.\treemon.ps1 deploy`, open a real changed worktree in both the canvas pane and a standalone tab. Exercise accordion switching, unified/split wrapping, highlighting, and every available change status. Fail the smoke run on any uncaught browser error, horizontal scrolling, incorrect or overlapping line numbers, multiple expanded patches, missing token highlighting for supported code, inaccessible control, or pane/standalone behavior mismatch.
