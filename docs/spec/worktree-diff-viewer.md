...
- The view loads a changed-file summary first, then restores the previous file selection or selects the first file. Changed files form a keyboard-operable, single-open accordion: for every non-empty ready summary, exactly one path header is expanded and exactly one patch is mounted. A clean worktree shows an explicit empty state.
