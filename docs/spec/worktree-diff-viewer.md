...
- The uniform 10-second Git deadline is measured through the complete API response. Argument-list execution uses a 9.25-second production budget, reserving up to 250 ms for process-tree termination and stream draining and 500 ms for process startup plus API response completion. Timeout results remain explicit and retry-oriented; operation-specific tuning should be introduced only if real-repository evidence shows the deadline is routinely too short.
