...
- Clicking an issue row expands a **detail panel** showing: full description, all labels, priority badge, type badge, dependency count, comment count, age
...
- **Add issue detail panel** — click a table row to expand a panel showing full description (rendered as text), labels as colored pills, priority/type badges, dependency/comment counts
...
- Template is stored as an embedded string or resource in the Server project
...
| `src/Server/BeadspaceTemplate.html` | Customized Beadspace template (embedded at build) |
