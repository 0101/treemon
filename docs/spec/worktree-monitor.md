...
- An in-process revision claim prevents concurrent delivery. After an agent prompt is accepted, the canonical worktree path, base revision, and acceptance time are also persisted in port-scoped gitignored runtime state at `data/auto-sync-{port}.json`; a missing, corrupt, or incomplete record loads as empty, so an unusable record re-prompts rather than suppressing a sync. The same revision is suppressed across restarts for one hour; a new revision triggers immediately. Catch-up, disable, merged-branch cleanup, and worktree removal clear the durable record.
