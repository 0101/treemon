...
- Archiving or deleting a worktree does not prune `autoSyncBranches`; branch-name reuse may restore the preference. When PR reconciliation identifies a merged PR, it removes the corresponding enabled local branch from `autoSyncBranches` and clears that worktree's accepted-revision record, so the card's toggle deactivates on the next dashboard refresh.
