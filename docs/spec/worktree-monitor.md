...
- Archiving or deleting a worktree does not prune `autoSyncBranches`; branch-name reuse may restore the preference. A reconciled merged PR is the exception: the PR refresh matches it by provider branch and removes the worktree's *local* branch from that repo's `autoSyncBranches` in one write per repo — a same-named branch in another repo is untouched — clears its trigger and durable accepted state, and the API rejects attempts to re-enable it while the merged state is known.
- Eligibility is the persisted preference *and* the absence of a known merged PR, and it is re-read immediately before an operation acts, since PR refresh runs on its own cadence. A branch disabled or reconciled merged after the observation that started the operation releases its revision claim and delivers nothing. Nothing is cancelled once a prompt has been accepted or a Git command is already running.
- An explicit enable re-reads the reconciled PR state after writing the preference, not only before: the PR refresh computes its merged cleanup from the preference as it stood *before* that write, so a merge reconciled mid-toggle would otherwise stay enabled until some later refresh. The enable owns the value it wrote — it rolls the preference back, clears the branch's trigger and durable accepted state, and answers with the same merged-branch error as a pre-checked rejection, so no enabled preference and no successful response survive the race.
- When enabled, fresh Git observations start a sync only when the worktree is behind a newly observed base revision. The canonical worktree path plus that base revision, not repeated polling of the same behind count, is the deduplication identity, and it outlives the process: an in-process claim prevents concurrent delivery within one server, while a durable accepted record suppresses the same revision across a restart. The two layers are coordinated, not independent: the claim distinguishes a delivery in flight from an accepted one, and only the durable record's expiry may retire an accepted claim — a delivery still in flight is never superseded, and an accepted claim never outlives the record that can age out. Acceptance publishes the refreshed record *before* the claim becomes retryable, and each acceptance carries an opaque generation that both layers record, so a clear retires exactly the acceptance whose record it read: a retry can never read a record older than the claim it took over, a cleared record can never leave a claim suppressing on its own, and a catch-up that was decided before a later acceptance can neither erase that acceptance's record nor retire its claim.
- An accepted agent prompt — and only an accepted one, so a rejection or a crash before acceptance retries — persists the canonical worktree path, base revision, acceptance time, and that acceptance's generation in port-scoped gitignored runtime state at `data/auto-sync-{port}.json`; a missing, corrupt, or incomplete record loads as empty, so an unusable record re-prompts rather than suppressing a sync, while a record written before generations existed loads with a fresh one, since the generation only pairs a record with a claim inside one process. The same revision stays suppressed for one hour, long enough to cover an in-progress sync; a different revision triggers immediately. Once the hour passes, that revision is prompted exactly once more — with or without a restart — and the new acceptance restarts the window; the record is re-read immediately before delivery, and the retrying prompt's own record is readable before its claim can be taken over, so concurrent triggers that started from the same expired record cannot prompt twice. Catch-up retires the acceptance it observed — the durable record and the accepted claim published under the same generation — while disable, merged-branch cleanup, and worktree removal end auto-sync for the worktree outright and forget both layers whatever acceptance wrote them, so falling behind the same revision again prompts instead of being blocked by a claim with no record left to age out.
...
- With no open session, Treemon first attempts a bounded Git-only sync. It merges only a worktree proven clean — local content, or a content probe that could not answer, both refuse — fetches `{upstreamRemote}/{baseBranch}` for that worktree, tries a fast-forward and then a non-editing merge, aborts conflicts, and verifies that the fetched base revision is an ancestor of `HEAD`. The sync is asked for one named branch and each merge attempt is bound to it: the branch is re-read from the worktree inside the Git path, after the cleanliness probe and the fetch and immediately before each merge command, so a checkout landing anywhere in that window refuses instead of merging the base into a branch nobody observed.
- After a successful mechanical sync, a live per-branch provider query decides the push rule: an open pull request gets a non-force push of the observed branch; no open pull request finishes locally; unknown PR state falls back to an agent. The observation, mechanical mutation, provider query, and push stay bound to one checked-out branch: the branch is re-read from the worktree before each merge, again before the provider is asked, and once more by the push itself, and any reading that is not the observed branch — including a detached `HEAD` — falls back instead, so a merge never lands on a branch nobody observed and one branch's pull-request state never authorizes pushing another. The push names that branch and the remote and branch git itself recorded as its upstream, as an explicit refspec, so neither a push default nor whatever `HEAD` points at picks what moves; the recorded remote is the one argument that must precede the refspec's `--`, so a recorded value starting with a dash — which git would read as an option rather than as a destination — is refused before the command is built. An unconfigured or option-like upstream, or a remote that has moved ahead, fails to the agent path rather than being forced. The query asks the branch's own provider — chosen by the same upstream-remote detection the dashboard uses — for that one branch's open pull requests, without threads, builds, or mergeability, and never reuses the eventually-consistent cached PR map. On GitHub this includes pull requests whose source branch lives in a fork: the head owner is read from the effective push URL of the remote git itself records for that branch — the same destination the push would use — rather than assumed to be the upstream repository's owner, and a branch with no resolvable GitHub head owner is unknown rather than an absence. Because that owner-and-branch filter is the only thing scoping the response, both halves of it are verified against what comes back — the owner case-insensitively, as GitHub logins are, and the branch exactly — so a pull request headed by another account's branch of the same name, or one whose head repository is gone, leaves the state unknown instead of authorizing a push. A command, authentication, timeout, payload, or unsupported-provider failure is unknown, never a confirmed absence. The mechanical path does not run project builds or tests.
- Dirty state, conflicts, failed aborts, command failures, a worktree that changed branches, unknown PR state, and failed pushes all fall back to one agent prompt containing only a closed structured reason. Raw Git output, paths, filenames, branch text, and commit messages never enter the prompt. Eligibility is re-read once more before that fallback prompt — a fetch, a merge, and a provider query have run since the pre-mutation check — so a branch disabled or reconciled merged meanwhile is dropped rather than handed to an agent.
- One per-worktree operation guard covers target selection, mechanical work, and delivery, so a later fetch cannot overlap an in-progress sync even if it observes a newer base revision. Only the operation that took the guard releases it, in a `finally`; worktree bookkeeping never prunes it, since a path can disappear from discovery while its sync is still merging and dropping the guard there would license an overlapping one.
