...
- Working-session-first auto-sync target: prefer the session mid-turn (background agents count as
  work), then the greatest-`UpdatedAt` open idle session, and use retained/offline identity only when
  nothing is open. Only the working case keeps the sync agent-driven — an idle or user-blocked
  session is a terminal left running, not an agent that will act — while still receiving any fallback
  prompt itself instead of triggering an unnecessary second session.
