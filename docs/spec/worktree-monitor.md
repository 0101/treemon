...
- Client optimistic state (`DeletedPaths: Set<string>`) filters by path for the browser session and
  is not cleared by dashboard refreshes or delete failures, so confirmed worktrees cannot reappear
  during that session.
