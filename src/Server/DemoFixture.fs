...
let private wtRetryLogic: WorktreeStatus =
    { Path = azDoPath "feature-retry"
      ...
      AgentIntent = Some("Adding jitter to the backoff delays", baseTimestamp.AddMinutes(-1.0))
...
let private wtConfigLoading: WorktreeStatus =
    { Path = azDoPath "refactor-config"
      ...
      AgentIntent = Some("Extracting env overrides into a typed loader", baseTimestamp.AddMinutes(-2.0))
...
let private wtAuthMiddleware: WorktreeStatus =
    { Path = azDoPath "feature-auth"
      ...
      AgentIntent = Some("Verifying the admin-role check on the delete endpoint", baseTimestamp.AddMinutes(-8.0))
...
let private wtStreaming: WorktreeStatus =
    { Path = githubPath "streaming"
      ...
      AgentIntent = Some("Wiring tumbling windows into the aggregator", baseTimestamp.AddSeconds(-30.0))
