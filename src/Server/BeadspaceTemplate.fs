...
function renderDashboard(issues) {
    ...
    return '<div id="view-dashboard" class="view active">' +
        '<div class="stats-row">' +
        '<div class="stat-card accent"><div class="stat-value" data-stat="total">' + total + '</div><div class="stat-label">Total</div></div>' +
        '<div class="stat-card open"><div class="stat-value" data-stat="open">' + open.length + '</div><div class="stat-label">Open</div></div>' +
        '<div class="stat-card wip"><div class="stat-value" data-stat="wip">' + wip.length + '</div><div class="stat-label">In Progress</div></div>' +
        '<div class="stat-card blocked"><div class="stat-value" data-stat="blocked">' + blocked.length + '</div><div class="stat-label">Blocked</div></div>' +
        '<div class="stat-card closed"><div class="stat-value" data-stat="closed">' + closed.length + '</div><div class="stat-label">Closed</div></div>' +
        '<div class="stat-card completion"><div class="stat-value" data-stat="completion">' + completionPct + '%</div><div class="stat-label">Completion</div></div>' +
        '</div>' +
...
function renderIssuesView() {
    ...
    return '<div id="view-issues" class="view">' +
        ...
        (tableState.query ? ' value="' + esc(tableState.query) + '"' : '') + '>' +
        chipsHtml +
        '</div>' +
...
function updateDashboard(issues) {
    ...
    // Update stat card values via data-stat attributes
    var statMap = {
        total: String(s.total),
        open: String(s.open.length),
        wip: String(s.wip.length),
        blocked: String(s.blocked.length),
        closed: String(s.closed.length),
    ...
    // Rebuild dashboard panels (triage, active, charts change shape too much for textContent)
    var dashView = document.getElementById('view-dashboard');
    if (dashView) {
        var wasActive = dashView.classList.contains('active');
...
// ─── Initial render: full DOM build + event binding (called once) ────────────
...
// ─── Incremental refresh: update data without destroying UI state ────────────
function refreshData() {
    fetch(dataUrl)
        ...
        .then(function(issues) {
            ...
            // Save scroll position of the issues table container
            var tbody = document.getElementById('issues-tbody');
            var scrollContainer = tbody ? tbody.closest('.view') || tbody.parentElement : null;
        ...
        .catch(function(err) {
            if (initialized) return; // Don't destroy UI on transient fetch errors after init
            var app = document.getElementById('app');
            app.textContent = '';
