...
let actionButton dispatch (wt: WorktreeStatus) (prompt: string) (title: string) (icon: ReactElement) =
        ...
        if pr.IsMerged then
            ...
            | WithResolution (unresolved, total) when total > 0 ->
                ...
                if unresolved > 0 then
                    actionButton dispatch wt $"/pr {pr.Url}" "Fix PR comments" commentIcon
            | CountOnly total ->
                Html.span [
            ...
            pr.Builds |> List.map (fun build ->
                React.fragment [
                    ...
                    if build.Status = Failed then
                        match build.Url with
                        | Some url -> actionButton dispatch wt $"/fix-build {url}" "Fix build" wrenchIcon
                        | None -> ()
                ]) |> React.fragment
    ]
...
let prRow dispatch (wt: WorktreeStatus) (repoName: string) =
    ...
    | NoPr, _ ->
        Html.div [
            ...
            prop.children [
                actionButton dispatch wt "Commit all changes, push to origin, and create a pull request for this branch" "Create PR" createPrIcon
            ]
        ]
    | HasPr pr, _ ->
        Html.div [
