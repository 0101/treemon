...
type Msg =
    ...
    // A canvas doc posted a valid object with no top-level string `action`, so CanvasPane could not
    // route it. Surfaced (not silently dropped) via the doc-error banner, attributed to the active
    // visible doc. `keys` are the message's top-level keys (logged for diagnostics in the pane).
    | CanvasMalformedDocMessage of keys: string
