...
type OverviewHistoryPublicationIntegrationTests() =
    ...
    [<Test>]
    member _.``startup exposes no raw or partial history before backfill succeeds``() =
        withDbPath (fun path ->
            let anchor =
            ...
            let sources =
                { emptySources with
                    Tasks = [ anchor.AddHours(-80.0), tc 5 ] }

            use store = new SessionActivityStore(path)
            persistSources store sources
            assertMissingPublication store

            // The failure is armed after one complete staging batch.
            let mutable stageNumber = 0

            let failingHooks =
                { noHooks with
                    BeforeStage =
                        fun _ ->
                            if Interlocked.Increment(&stageNumber) = 2 then
                                raise (
                                    InvalidOperationException(
                                        "forced startup reconstruction failure"
                                    )
                                ) }
            ...
            (use failingWorker =
             ...
             let error =
                 Assert.Throws<InvalidOperationException>(fun () ->
                     Program.initializeOverviewHistory failingWorker
                     |> ignore)

             Assert.That(
                 error.Message,
                 Is.EqualTo "forced startup reconstruction failure"
             ))
            ...
            let publication =
                Program.initializeOverviewHistory worker
            ...
            Assert.Multiple(fun () ->
                Assert.That(
                    ...
                    Is.EqualTo publication.SourceGeneration
                )
                Assert.That(
                    publication.CompleteThrough,
                    Is.EqualTo(Some anchor)
                )
...
"""
    ...
    [<Test>]
    member _.``derived schema reset rebuilds raw sources before API availability``() =
        withDbPath (fun path ->
            let anchor =
                latestCompleteBoundary(
                    DateTimeOffset.UtcNow.AddMinutes 5.0
            ...
            let sources =
                { Tasks = [ anchor.AddHours(-80.0), tc 2 ]
                  Events =
                    [ evt
                        "schema-working"
                        "schema-session"
...
"""
            ...
            assertMissingPublication rebuilt

            use worker =
                createWorker rebuilt (fixedClock anchor) noHooks
            ...
            Assert.Multiple(fun () ->
                ...
                Assert.That(
                    scalarInt
                        path
                        "SELECT count(*) FROM overview_history_session_bounds;",
                    Is.EqualTo 1
                )
                Assert.That(
                    (readApi rebuilt HistoryWindow.Hours72).Anchor,
                    Is.EqualTo anchor
                ))

            assertPublishedMatchesOracle
                "derived-schema rebuild"
                rebuilt
                (oldestExposedBoundary anchor)
                anchor
                sources)
