...
type DiffProvisioningTests() =
    ...
    member _.``diff viewer is provisioned and synchronized from the embedded template``() =
        let first = DiffProvisioner.provisionViewer tempDir true
        let diffPath = Path.Combine(tempDir, ".agents", "canvas", "diff.html")
        let second = DiffProvisioner.provisionViewer tempDir true
        ...
        let third = DiffProvisioner.provisionViewer tempDir true
    ...
    member _.``a worktree without comparison content gets no viewer``() =
        ...
        let clean = DiffProvisioner.provisionViewer tempDir false
        DiffProvisioner.provisionViewer tempDir true |> ignore
        let removal = DiffProvisioner.provisionViewer tempDir false
    ...
    member _.``losing comparison content preserves a document the user authored at the viewer path``() =
        ...
        let outcome = DiffProvisioner.provisionViewer tempDir false
    ...
    member _.``provisioning preserves a document the user authored at the viewer path``() =
        ...
        let outcome = DiffProvisioner.provisionViewer tempDir true
