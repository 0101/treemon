...
type DiffProvisioningTests() =
    ...
    member _.``diff viewer is provisioned and synchronized from the embedded template``() =
        let first = DiffProvisioner.provisionViewer tempDir GitWorktree.HasContent |> TestUtils.runAsync
        let diffPath = Path.Combine(tempDir, ".agents", "canvas", "diff.html")
        let second = DiffProvisioner.provisionViewer tempDir GitWorktree.HasContent |> TestUtils.runAsync
        ...
        let third = DiffProvisioner.provisionViewer tempDir GitWorktree.HasContent |> TestUtils.runAsync
    ...
    [<Test>]
    member _.``a worktree without comparison content gets no viewer``() =
        let diffPath = Path.Combine(tempDir, ".agents", "canvas", "diff.html")

        let clean = DiffProvisioner.provisionViewer tempDir GitWorktree.Clean |> TestUtils.runAsync
        DiffProvisioner.provisionViewer tempDir GitWorktree.HasContent |> TestUtils.runAsync |> ignore
        let removal = DiffProvisioner.provisionViewer tempDir GitWorktree.Clean |> TestUtils.runAsync

        Assert.Multiple(fun () ->
            Assert.That(clean, Is.EqualTo(None), "Nothing to remove is not worth reporting")
            Assert.That(File.Exists(diffPath), Is.False)
            Assert.That(removal.IsSome, Is.True))

    [<Test>]
    member _.``a worktree whose comparison could not be evaluated keeps its viewer``() =
        let diffPath = Path.Combine(tempDir, ".agents", "canvas", "diff.html")

        let provisioned = DiffProvisioner.provisionViewer tempDir GitWorktree.Undetermined |> TestUtils.runAsync
        let repeated = DiffProvisioner.provisionViewer tempDir GitWorktree.Undetermined |> TestUtils.runAsync

        Assert.Multiple(fun () ->
            Assert.That(provisioned.IsSome, Is.True)
            Assert.That(repeated, Is.EqualTo(None))
            Assert.That(
                File.Exists(diffPath),
                Is.True,
                "An unevaluated comparison must keep the page that renders the base-error state"))

    [<Test>]
    member _.``losing comparison content preserves a document the user authored at the viewer path``() =
        let diffPath = Path.Combine(tempDir, ".agents", "canvas", "diff.html")
        ...
        let outcome = DiffProvisioner.provisionViewer tempDir GitWorktree.Clean |> TestUtils.runAsync

        Assert.Multiple(fun () ->
            Assert.That(
                File.ReadAllText(diffPath),
                Is.EqualTo(authored),
                "Removing the generated viewer must never delete an authored document")
            Assert.That(outcome.IsSome, Is.True))
    ...
    member _.``provisioning preserves a document the user authored at the viewer path``() =
        ...
        let outcome = DiffProvisioner.provisionViewer tempDir GitWorktree.HasContent |> TestUtils.runAsync
